...
### 2.1 WAN за NAT — когда работает, когда нет

OPNsense с AmneziaWG-клиентом часто оказывается за NAT — за домашним/офисным роутером, за гипервизором, за CGN провайдера, или сразу за несколькими (двойной/тройной NAT). Это **не запрещено**, но добавляет один риск: UDP NAT-state на промежуточном устройстве должен жить достаточно долго, чтобы handshake и ответный трафик не теряли обратный путь.

**Как это работает.** AmneziaWG/WireGuard — stateful UDP, ответы должны попадать в тот же NAT-mapping, что и запрос. Чтобы mapping не успел истечь между пакетами, в `.conf` есть параметр `PersistentKeepalive = 25` — клиент шлёт keepalive UDP-пакет каждые 25 секунд независимо от пользовательского трафика, чем держит mapping живым.

**Проверка по умолчанию.** Большинство домашних/SOHO-роутеров (MikroTik, Asus, Keenetic, OpenWRT, OPNsense/pfSense в роли upstream) держат UDP-state 60–180 секунд из коробки. `PersistentKeepalive = 25` их перебивает с большим запасом — двойной/тройной NAT в типичном домашнем/офисном сценарии работает прозрачно.

**Когда NAT всё-таки ломает.**

- **`PersistentKeepalive` отсутствует или больше UDP-таймаута** одного из устройств на пути. Современные `.conf` от Amnezia-серверов почти всегда содержат `PersistentKeepalive = 25`, но проверь свой.
- **CGN-провайдер с агрессивным UDP-таймаутом (~30 сек)**. Встречается у мобильных операторов и низкобюджетных проводных провайдеров. Handshake может не успевать в окно. См. §2.2.
- **Симметричный NAT с port-randomization**. Каждое исходящее UDP-соединение получает свежий внешний порт, входящие от сервера пакеты летят в «никуда». Это редкий тип NAT, но если попался — никакой keepalive не поможет, нужен другой провайдер или публичный IP.
- **Hyper-V Default Switch**. У микрософтовского Default Switch собственная NAT-реализация, которая криво работает со stateful UDP даже при keepalive — отдельная история, см. §3.1.

**Симптомы.** Если в `awg show awg0` видишь `0 B received, X KiB sent` и нет `latest handshake` — это типичная картина того, что handshake не доходит. Тогда проверить keepalive в `.conf`, тип провайдерского WAN и (если виртуалка на Hyper-V) — Default Switch.

**Лечение по ситуации.**
- В `.conf` отсутствует `PersistentKeepalive` — добавить `PersistentKeepalive = 25`.
- На Hyper-V — переключить WAN на External Switch (см. §3.1).
- На физическом OPNsense за CGN провайдера с коротким UDP-таймаутом — попросить публичный IP или использовать другого провайдера.
